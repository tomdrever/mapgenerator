...
function diamondSquare(DATA_SIZE) {
	...
	//seed the data
	data[0][0] = data[0][DATA_SIZE - 1] = data[DATA_SIZE - 1][0] = data[DATA_SIZE - 
	...
	for (var sideLength = DATA_SIZE - 1;
		...
		for (var x = 0; x < DATA_SIZE - 1; x += sideLength) {
		    for (var y = 0; y < DATA_SIZE - 1; y += sideLength) {
		        ...
		        data[x + halfSide][y + halfSide] = avg + (Math.random() * 2 * h) - 
...
h;
		...
		for (var x = 0; x < DATA_SIZE - 1; x += halfSide) {
		    ...
		    //to generate the far edge values
		    for (var y = (x + halfSide) % sideLength; y < DATA_SIZE - 1; y += 
...
sideLength) {
		        ...
		        var avg =
                    data[(x - halfSide + DATA_SIZE - 1) % (DATA_SIZE - 1)][y] + //left of 

center
                    data[(x + halfSide) % (DATA_SIZE - 1)][y] + //right of center
                    data[x][(y + halfSide) % (DATA_SIZE - 1)] + //below center
                    data[x][(y - halfSide + DATA_SIZE - 1) % (DATA_SIZE - 1)]; //above 
